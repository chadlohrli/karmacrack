...
contract KarmaGang {
    ...
    uint160  public uint160address;
    uint160  public shifted;
    ...
    function testuint160() external {
        uint160address = uint160(msg.sender);
        shifted = uint160address >> (8*17);
    }

    function testAddress(address addy) external returns (bool) {
        return uint160(addy) >> (8 * 17) == 0xc0de42;
    }

    function testAddress1(address addy) external returns (uint160) {
        return uint160(addy) >> (8 * 17);
    }
